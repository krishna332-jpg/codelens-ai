...
function AuthPage() {
  ...
  const handleGoogle = async function() {
    ...
    try {
      ...
      setError(err.code ? err.code + ': ' + err.message : 'Sign-in failed. Please try again.');
  ...
  return (
    <div className="auth-overlay">
      <div className="auth-card">
        ...
        <button className="auth-google" onClick={handleGoogle} disabled={loading} style={{padding:'16px', fontSize:'15px', fontWeight:'700'}}>
          {loading ? 'Signing in...' : (
            <>
        ...
        <p style={{textAlign:'center', fontSize:'12px', color:'#999', marginTop:'20px', lineHeight:'1.6'}}>
          By continuing, you agree to our terms of service.<br/>
          Your Google account will be used to sign in securely.
        </p>
      </div>
    </div>
  );
